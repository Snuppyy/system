        ...
        $(function () {
            ...
            $('#filterDate').on('apply.daterangepicker', function (ev, picker) {
                @if(!isset($_GET['region']))
                    window.location.href = "{{ route('view-MiOVisitions') }}?startDate=" + picker.startDate.format('YYYY-MM-DD') + "&endDate=" + picker.endDate.format('YYYY-MM-DD');
                @else
                    window.location.href = "{{ route('view-MiOVisitions', ['region' => $_GET['region']]) }}&startDate=" + picker.startDate.format('YYYY-MM-DD') + "&endDate=" + picker.endDate.format('YYYY-MM-DD');
                @endif
            });
...
@section('header')
    <div class="sh-breadcrumb">
        <nav class="breadcrumb">
            ...
            <span class="breadcrumb-item active">Мониторинговые визиты</span>
        </nav>
    </div><!-- sh-breadcrumb -->
    ...
    <div class="sh-pagetitle">
        ...
        <div class="sh-pagetitle-left">
            ...
            <div class="sh-pagetitle-title">
                <h2>Зарегистрированные мониторинговые визиты</h2>
...
@section('content')
            ...
            <div class="pd-10 bd mg-b-20">
                <ul class="nav nav-pills flex-column flex-md-row justify-content-center" role="tablist">
                    <li class="nav-item"><a class="nav-link{{ !isset($_GET['region']) ? ' active' : ''}}"
                                            ...
                                            href="{{ route('view-MiOVisitions', ['startDate' => $_GET['startDate'], 'endDate' => $_GET['endDate']]) }}"
                                            @else
                                            href="{{ route('view-MiOVisitions') }}"
                                            @endif
                                            role="tab">Все</a></li>
                    @foreach($regions as $region)
                        @if($region->id !== 0)
                            <li class="nav-item"><a
                                        ...
                                        @if(isset($_GET['startDate']) && isset($_GET['endDate']))
                                        href="{{ route('view-MiOVisitions', ['region' => $region->encoding, 'startDate' => $_GET['startDate'], 'endDate' => $_GET['endDate']]) }}"
                                        @else
                                        href="{{ route('view-MiOVisitions', ['region' => $region->encoding]) }}"
                                        @endif
    ...
    <a href="{{ route('view-MiOVisitions', ['filter' => 'duplicates']) }}"
       class="text-center card card-body btn btn-danger bg-danger"
    ...
    <a
            ...
            @if(isset($_GET['region'])))
            href="{{ route('view-MiOVisitions', ['region' => $_GET['region']]) }}"
            @else
            href="{{ route('view-MiOVisitions', ['region' => $_GET['region']]) }}"
            @endif
    >
    ...
    <table class="table table-hover table-bordered table-primary mg-b-0">
        ...
        @foreach($data as $miovisition)
            <tr>
                ...
                @if(($miovisition->user_id === auth()->user()->id || $miovisition->author_id === auth()->user()->id || auth()->user()->role <= 2) && ($miovisition->status === 1))
                    @if(!$miovisition->scan)
                        <td>
                            <form action="{{ route('upload-documents', ['type' => 'mio', 'id' => $miovisition->id]) }}"
                                  method="POST" enctype="multipart/form-data" class="scanUploader">
                                @csrf
                                <a href="{{ route('get-MiOVisitions', $miovisition->id) }}"
                                   class="fa fa-pencil-square text-warning"></a>
                                <a href="{{ route('delete-MiOVisitions', $miovisition->id) }}"
                    ...
                    @else
                        <td>
                            ...
                            <a href="{{ route('delete-MiOVisitions', $miovisition->id) }}"
                               class="fa fa-trash text-danger"></a>
                            <a href="{{ $miovisition->scan }}"
                               class="fa fa-file-image-o text-success preview"></a>
                        </td>
                    @endif
                @elseif($miovisition->status !== 1)
                    <td>
