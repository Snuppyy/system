...
@section('content')
    <div class="wd-100p alert alert-success mg-10 text-center">
        Всего сотрудников: {{ $countUsers }}<br>
        Всего должностей: {{ $countPositions }}<br>
    </div>
    @foreach($projects as $project)
        <div class="col-6 col-sm-4 col-md">
            <a href="{{ route('ActivityStatisticsProject', ['project' => $project->id]) }}" class="shortcut-icon {{ $project->status === 0 ? ' disabled' : '' }}">
                <div>
