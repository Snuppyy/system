...
@section('content')
    ...
    @endif
    <form autocomplete="off" action="{{ route('update-questionnaires', ['encoding' => $questionnaire['encoding'], 'id' => $answers->id]) }}" method="post"
                ...
                <div class="form-group mg-b-10-force sel2">
                    ...
                    <select id="outreach" class="form-control select2 sel2"
                        ...
                        @foreach($outreaches as $outreach)
                            @if($outreach->id === $answers->outreach)
                                <option selected
                                        value="{{ $outreach->id }}">{{ $outreach->encoding ? $outreach->encoding.' - '.$outreach->name : $outreach->name }}</option>
                            @else
            ...
            <div class="col-lg-2">
                <div class="form-group">
                    ...
                    <div id="typeClient" class="form-control">
                        <label class="rdiobox">
                            <input {{ $answers->type === 1 ? 'checked ' : '' }}name="type" value="1" type="radio"
                        ...
                        <label class="rdiobox mg-b-0-force">
                            <input {{ $answers->type === 2 ? 'checked ' : '' }}name="type" value="2" type="radio"
                                   data-parsley-trigger="change" required>
                            <span>ПОСТ</span>
                        </label>
                    </div>
                        ...
                        <div class="form-group mg-b-10-force pd-t-10">
                                ...
                                <div class="form-control {{ $questionnaire['encoding'].'_'.$id_question }}">
                                    <label class="rdiobox mg-b-0-force">
                                        <input name="{{ $questionnaire['encoding'].'_'.$id_question }}"
                                               @if($answers->answers[$id_question] == $id_answer)
                                               checked
                                               @endif
                                               value="{{ $id_answer }}" type="radio"
                            ...
                            <div class="form-control {{ $questionnaire['encoding'].'_'.$id_question }}">
                                <label class="rdiobox mg-b-0-force">
                                    <input name="{{ $questionnaire['encoding'].'_'.$id_question }}"
                                           value="0" type="radio"
                                           @if($answers->answers[$id_question] == 0)
                                           checked
                                           @endif
                                           data-parsley-class-handler=".{{ $questionnaire['encoding'].'_'.$id_question }}"
                                           data-parsley-errors-container="#{{ $questionnaire['encoding'].'_'.$id_question }}ErrorContainer"
