    ...
    <script>
        $(function () {
            ...
            $('#user').change(function () {
                ...
                var href = '{{ route('tempRegistrationUser', ['user' => $user, 'position' => ':position']) }}';
                href = href.replace(':position', $(this).val());
...
@section('content')
    <div class="col-lg-12">
        <div class="form-group mg-b-10-force sel2">
            <label for="user" class="form-control-label">Должность: <span
            ...
            <select id="user" class="form-control select2 sel2 wd-100p-force"
                ...
                @foreach($positions as $position)
                    <option value="{{ $position->id }}">{{ $position->name_ru }}</option>
                @endforeach
            </select>
        </div>
    </div><!-- col-4 -->
