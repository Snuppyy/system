...
$.fn.EZView =  function(collectionName) {
    ...
    // if (!$(self).length) {
    //     throw Error('Нет загруженных элементов');
    // }
        ...
        if (!$('#EZView' + collectionName).length) {
            ...
            var tools = 
                ...
                template = '<div id="' + collectionName + '" ' + 
                           'class="EZView container-viewer hide" style="display: flex; align-items: center; justify-content: center">' +
                           container + '</div>';
    ...
    self.buildHtmlContent = function() {
        ...
        // Content to show 
        var htmlContent = '<img index-content="' + collectionName + imgIndex + '" src="' + src + '" class="content-viewer" />';

        // To show pdf files
        if (isPdf) {
            htmlContent = '<iframe class="content-viewer" frameborder="0" index-content="' + collectionName + imgIndex +
    ...
    self.setContentOnViewer = function(htmlContent) {
        ...
        $('[index-content=' + collectionName + newIndex + ']').on( "error",function() {
            ...
            $(this).replaceWith('<h1 class = "content-viewer" index-content="' + collectionName + self.index[collectionName] + '" style="' + style + '">Unsupported preview</h1>');
    ...
    self.goTo = function(newIndex) {
        // Hide all imgs
        $('.content-viewer', self.$EZView).hide();
